...
safer_roads_recommendation = pd.read_csv(io.BytesIO(uploaded['safer roads recommendation.csv']))
safer_roads_recommendation.head(10)
print(
safer_roads_recommendation)

safer_roads_recommendation.head()
...
# add markers to map
for lat, lng, label in zip(safer_roads_recommendation['latitude'], safer_roads_recommendation['longitude'], safer_roads_recommendation['Road recommendation ']):
    label = folium.Popup(label, parse_html=True)
    folium.CircleMarker(
        [lat, lng],
        radius=5,
...
#one hot encoding
safer_roads_recommendation_onehot = pd.get_dummies(safer_roads_recommendation[['Type of danger']], prefix = "", prefix_sep ="" )

#add neighborhood column back to dataframe
safer_roads_recommendation_onehot ['Neighborhood'] = safer_roads_recommendation ['Neighborhood']

#move neighborhood column to the first column
fixed_columns = [safer_roads_recommendation_onehot.columns[-1]] + list(safer_roads_recommendation_onehot.columns[:-1])
safer_roads_recommendation_onehot = safer_roads_recommendation_onehot[fixed_columns]

safer_roads_recommendation_onehot.tail()

safer_roads_recommendation_onehot.shape

#to be used while generating graphs
safer_roads_recommendation_grouped = safer_roads_recommendation_onehot.groupby('Neighborhood').mean().reset_index()
safer_roads_recommendation_grouped.tail()

for i in safer_roads_recommendation_grouped.columns:
  print (i, end=", ")

safer_roads_recommendation_grouped.shape
...
neighborhoods_dangers_sorted['Neighborhood'] = safer_roads_recommendation_grouped['Neighborhood']

for ind in np.arange(safer_roads_recommendation_grouped.shape[0]):
    neighborhoods_dangers_sorted.iloc[ind, 1:] = return_most_common_dangers(safer_roads_recommendation_grouped.iloc[ind, :], num_top_dangers)
...
safer_roads_recommendation_grouped_clustering = safer_roads_recommendation_grouped.drop ('Neighborhood', 1)

#run K-means clustering
kmeans = KMeans(n_clusters=kclusters, random_state=0).fit(safer_roads_recommendation_grouped_clustering)

#check cluster labels generated for each row in the dataframe
kmeans.labels_[0:10]

# add clustering labels
neighborhoods_dangers_sorted.insert(0, 'Labels', kmeans.labels_)

safer_roads_recommendation_merged = jakarta_neighborhood_data

# merge toronto_grouped with toronto_data to add latitude/longitude for each neighborhood
safer_roads_recommendation_merged = safer_roads_recommendation_merged.join(neighborhoods_dangers_sorted.set_index('Neighborhood'), on='Neighborhood')

safer_roads_recommendation_merged.dropna(inplace=True)
safer_roads_recommendation_merged.tail() # check the last columns!
...
markers_colors = []
for lat, lon, poi, cluster in zip(safer_roads_recommendation_merged['latitude'], safer_roads_recommendation_merged['longitude'], safer_roads_recommendation_merged['Neighborhood'], safer_roads_recommendation_merged['Cluster Labels']):
...
clusterdata = pd.merge(safer_roads_recommendation_onehot.groupby('Neighborhood').sum(),safer_roads_recommendation_merged[['Neighborhood','Cluster Labels']],left_on='Neighborhood', right_on='Neighborhood',how='inner')
...
safer_roads_recommendation.drop(safer_roads_recommendation.filter(regex="Unname"),axis=1, inplace=True)

del safer_roads_recommendation['No']
...
forheatmap=safer_roads_recommendation.copy()
forheatmap=pd.merge(forheatmap,safer_roads_recommendation_merged[['Neighborhood','Cluster Labels']],left_on='Neighborhood', right_on='Neighborhood',how='inner')
...
forrsk = forheatmap[forheatmap['Type of danger']=='Jalan Rusak']

# heat map for Begal in the selected Neighborhoods
rec_heat_rsk = folium.Map(location=[latitude, longitude], zoom_start=11)
HeatMap(list(zip(forrsk['latitude'],forrsk['longitude'])),
        min_opacity=0.7,
        radius=10, blur=15,
        max_zoom=1
       ).add_to(rec_heat_rsk)
for lat, lng, label in zip(forrsk['Neighborhood latitude'], forrsk['Neighborhood longitude'], forrsk['Neighborhood']):
    label = folium.Popup(label, parse_html=True)
    folium.CircleMarker(
        [lat, lng],
        radius=5,
        ...
        fill_color='#cc4831',
        fill_opacity=0.7,
        parse_html=False).add_to(rec_heat_rsk)
rec_heat_rsk
...
count_rsk = forheatmap[forheatmap['Type of danger']=="Jalan Rusak"][['Neighborhood','Road recommendation ']].groupby('Neighborhood').count().sort_values(by='Road recommendation ')
target_count = int(0.4*len(count_rsk))
print(count_rsk.iloc[target_count])
count_rsk.drop
count_rsk.columns = ['rsk count']
count_rsk.head()
...
temp_recommend.drop(columns=['index','Neighborhood'], inplace=True)
...
final_recommend=roadNearMenteng.append(roadNearPalmerah).append(roadNearDurenSawit).reset_index()
